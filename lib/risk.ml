...
let country_check country =
  ...
  | "uae" | "hong kong" ->
      (25, ["elevated country risk"])
  | _ ->
      (0, [])

let amount_check amount =
  if amount >= 1_000_000.0 then
    (30, ["high transaction amount"])
  else if amount >= 250_000.0 then
    (15, ["moderate transaction amount"])
  else
...
let entity_match_check score =
  if score >= 0.95 then
    (0, [])
  else if score >= 0.80 then
    (25, ["moderate entity match confidence"])
  else
    (60, ["low entity match confidence"])

let stamp_check blurry =
  if blurry then
    (20, ["blurry or unclear stamp"])
  else
    (0, [])

let sanctions_check flagged =
  if flagged then
    (100, ["explicit sanctions flag"])
  else
    (0, [])
